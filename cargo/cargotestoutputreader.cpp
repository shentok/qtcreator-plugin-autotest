...
static QString constructBenchmarkInformation(const QString &metric, double value, int iterations)
{
    ...
    return CargoTestOutputReader::tr("%1 %2 per iteration (total: %3, iterations: %4)")
...
CargoTestOutputReader::CargoTestOutputReader(const QFutureInterface<TestResultPtr> &futureInterface,
...
void CargoTestOutputReader::processOutputLine(const QByteArray &outputLine)
{
...
TestResultPtr CargoTestOutputReader::createDefaultResult() const
{
    CargoTestResult *result = new CargoTestResult(id(), m_projectFile, m_testType, m_className);
...
{
    return CargoTestOutputReader::tr("Qt version: %1").arg(version);
}

static QString trQtBuild(const QString &build)
{
    return CargoTestOutputReader::tr("Qt build: %1").arg(build);
}

static QString trQtestVersion(const QString &test)
{
    return CargoTestOutputReader::tr("QTest version: %1").arg(test);
}

void CargoTestOutputReader::processXMLOutput(const QByteArray &outputLine)
{
...
void CargoTestOutputReader::processPlainTextOutput(const QByteArray &outputLine)
{
...
void CargoTestOutputReader::processResultOutput(const QString &result, const QString &message)
{
...
void CargoTestOutputReader::processLocationOutput(const QString &fileWithLine)
{
...
void CargoTestOutputReader::processSummaryFinishOutput()
{
...
void CargoTestOutputReader::sendCompleteInformation()
{
...
void CargoTestOutputReader::sendMessageCurrentTest()
{
    CargoTestResult *testResult = new CargoTestResult(QString(), m_projectFile, m_testType, QString());
...
void CargoTestOutputReader::sendStartMessage(bool isFunction)
{
    TestResultPtr testResult = createDefaultResult();
...
void CargoTestOutputReader::sendFinishMessage(bool isFunction)
{
    TestResultPtr testResult = createDefaultResult();
...
void CargoTestOutputReader::handleAndSendConfigMessage(const QRegularExpressionMatch &config)
{
    TestResultPtr testResult = createDefaultResult();
