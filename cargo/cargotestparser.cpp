...
#include "cargotestparser.h"
#include "cargotesttreeitem.h"
...
TestTreeItem *CargoTestParseResult::createTestTreeItem() const
{
    if (itemType == TestTreeItem::Root)
        return nullptr;

    CargoTestTreeItem *item = new CargoTestTreeItem(framework, displayName, fileName, itemType);
...
static bool includesQtTest(const CPlusPlus::Document::Ptr &doc, const CPlusPlus::Snapshot &snapshot)
{
    static QStringList expectedHeaderPrefixes
            ...
            ? QStringList({"cargotest.framework/Headers", "cargotest"}) : QStringList({"cargotest"});
    ...
    for (const CPlusPlus::Document::Include &inc : includes) {
        ...
        if (inc.unresolvedFileName() == QString("cargotest")) {
                         ...
                         const CPlusPlus::Snapshot &snapshot, const QString &fileName)
{
    ...
    for (const CPlusPlus::Document::MacroUse &macro : macros) {
        ...
        if (CargoTestUtils::isQTestMacro(name) && !macro.arguments().isEmpty()) {
...
static QSet<QString> filesWithDataFunctionDefinitions(
            const QMap<QString, CargoTestCodeLocationAndType> &testFunctions)
{
    QSet<QString> result;
    QMap<QString, CargoTestCodeLocationAndType>::ConstIterator it = testFunctions.begin();
    const QMap<QString, CargoTestCodeLocationAndType>::ConstIterator end = testFunctions.end();
...
static QHash<QString, CargoTestCodeLocationList> checkForDataTags(const QString &fileName,
            const CPlusPlus::Snapshot &snapshot)
{
    const QByteArray fileContent = CppParser::getFileContent(fileName);
 ...
 */
static bool containsFunction(const QMap<QString, CargoTestCodeLocationAndType> &testFunctions,
...
static void mergeTestFunctions(QMap<QString, CargoTestCodeLocationAndType> &testFunctions,
                               const QMap<QString, CargoTestCodeLocationAndType> &inheritedFunctions)
{
    static const QString dataSuffix("_data");
    // take over only inherited test functions that have not been re-implemented
    QMap<QString, CargoTestCodeLocationAndType>::ConstIterator it = inheritedFunctions.begin();
    QMap<QString, CargoTestCodeLocationAndType>::ConstIterator end = inheritedFunctions.end();
    for ( ; it != end; ++it) {
...
static void fetchAndMergeBaseTestFunctions(const QSet<QString> &baseClasses,
                                           QMap<QString, CargoTestCodeLocationAndType> &testFunctions,
...
static CargoTestCodeLocationList tagLocationsFor(const CargoTestParseResult *func,
                                              const QHash<QString, CargoTestCodeLocationList> &dataTags)
{
    if (!func->inherited())
        return dataTags.value(func->name);

    QHash<QString, CargoTestCodeLocationList>::ConstIterator it = dataTags.begin();
    QHash<QString, CargoTestCodeLocationList>::ConstIterator end = dataTags.end();
    ...
    return CargoTestCodeLocationList();
}
                         ...
                         ITestFramework *framework)
{
    ...
    if (!testCaseName.isEmpty()) {
        ...
        QMap<QString, CargoTestCodeLocationAndType> testFunctions = visitor.privateSlots();
        ...
        QHash<QString, CargoTestCodeLocationList> dataTags;
        for (const QString &file : files)
            Utils::addToHash(&dataTags, checkForDataTags(file, snapshot));

        CargoTestParseResult *parseResult = new CargoTestParseResult(framework);
        ...
        const QMap<QString, CargoTestCodeLocationAndType>::ConstIterator end = testFunctions.end();
        for ( ; it != end; ++it) {
            const CargoTestCodeLocationAndType &location = it.value();
            QString functionName = it.key();
            functionName = functionName.mid(functionName.lastIndexOf(':') + 1);
            CargoTestParseResult *func = new CargoTestParseResult(framework);
            ...
            const CargoTestCodeLocationList &tagLocations = tagLocationsFor(func, dataTags);
            for (const CargoTestCodeLocationAndType &tag : tagLocations) {
                CargoTestParseResult *dataTag = new CargoTestParseResult(framework);
...
void CargoTestParser::init(const QStringList &filesToParse, bool fullParse)
{
    if (!fullParse) { // in a full parse cached information might lead to wrong results
        m_testCaseNames = CargoTestUtils::testCaseNamesForFiles(framework(), filesToParse);
        m_alternativeFiles = CargoTestUtils::alternativeFiles(framework(), filesToParse);
    }
    CppParser::init(filesToParse, fullParse);
}

void CargoTestParser::release()
{
...
bool CargoTestParser::processDocument(QFutureInterface<TestParseResultPtr> futureInterface,
