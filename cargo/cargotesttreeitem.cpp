...
CargoTestTreeItem::CargoTestTreeItem(ITestFramework *framework, const QString &name,
...
TestTreeItem *CargoTestTreeItem::copyWithoutChildren()
{
    CargoTestTreeItem *copied = new CargoTestTreeItem(framework());
    copied->copyBasicDataFrom(this);
    copied->m_inherited = m_inherited;
    return copied;
}

QVariant CargoTestTreeItem::data(int column, int role) const
{
    switch (role) {
    case Qt::DisplayRole:
        if (type() == Root)
...
Qt::ItemFlags CargoTestTreeItem::flags(int column) const
{
...
bool CargoTestTreeItem::canProvideTestConfiguration() const
{
    switch (type()) {
    case TestCase:
    case TestFunction:
...
bool CargoTestTreeItem::canProvideDebugConfiguration() const
{
    return canProvideTestConfiguration();
}

TestConfiguration *CargoTestTreeItem::testConfiguration() const
{
    ProjectExplorer::Project *project = ProjectExplorer::SessionManager::startupProject();
    QTC_ASSERT(project, return nullptr);

    CargoTestConfiguration *config = nullptr;
    switch (type()) {
    case TestCase:
        config = new CargoTestConfiguration(framework());
                                                 ...
                                                 QList<TestConfiguration *> &testConfigurations)
{
    ...
    CargoTestConfiguration *testConfig = nullptr;
    switch (item->checked()) {
    case Qt::Unchecked:
        return;
    case Qt::Checked:
        testConfig = static_cast<CargoTestConfiguration *>(item->testConfiguration());
    ...
    case Qt::PartiallyChecked:
        ...
        testConfig = new CargoTestConfiguration(item->framework());
...
static void collectFailedTestInfo(TestTreeItem *item, QList<TestConfiguration *> &testConfigs)
{
    ...
    CargoTestConfiguration *testConfig = new CargoTestConfiguration(item->framework());
    testConfig->setTestCases(testCases);
    testConfig->setProjectFile(item->proFile());
    testConfig->setProject(ProjectExplorer::SessionManager::startupProject());
    testConfig->setInternalTargets(item->internalTargets());
...
TestConfiguration *CargoTestTreeItem::debugConfiguration() const
{
    CargoTestConfiguration *config = static_cast<CargoTestConfiguration *>(testConfiguration());
    if (config)
        config->setRunMode(TestRunMode::Debug);
    return config;
}

QList<TestConfiguration *> CargoTestTreeItem::getAllTestConfigurations() const
{
...
QList<TestConfiguration *> CargoTestTreeItem::getSelectedTestConfigurations() const
{
    QList<TestConfiguration *> result;
    ProjectExplorer::Project *project = ProjectExplorer::SessionManager::startupProject();
    if (!project || type() != Root)
...
QList<TestConfiguration *> CargoTestTreeItem::getFailedTestConfigurations() const
{
    QList<TestConfiguration *> result;
...
QList<TestConfiguration *> CargoTestTreeItem::getTestConfigurationsForFile(const Utils::FilePath &fileName) const
{
    QList<TestConfiguration *> result;
...
TestTreeItem *CargoTestTreeItem::find(const TestParseResult *result)
{
    ...
    case GroupNode:
        return findChildByFile(result->fileName);
    case TestCase: {
        const CargoTestParseResult *qtResult = static_cast<const CargoTestParseResult *>(result);
...
TestTreeItem *CargoTestTreeItem::findChild(const TestTreeItem *other)
{
    ...
    case TestCase: {
        if (otherType != TestFunction && otherType != TestDataFunction && otherType != TestSpecialFunction)
            return nullptr;
        auto qtOther = static_cast<const CargoTestTreeItem *>(other);
...
bool CargoTestTreeItem::modify(const TestParseResult *result)
{
...
TestTreeItem *CargoTestTreeItem::createParentGroupNode() const
{
    const QFileInfo fileInfo(filePath());
    const QFileInfo base(fileInfo.absolutePath());
    return new CargoTestTreeItem(framework(), base.baseName(), fileInfo.absolutePath(), TestTreeItem::GroupNode);
}

bool CargoTestTreeItem::isGroupable() const
{
    return type() == TestCase;
}

TestTreeItem *CargoTestTreeItem::findChildByNameAndInheritance(const QString &name, bool inherited) const
{
    return findFirstLevelChild([name, inherited](const TestTreeItem *other) {
        const CargoTestTreeItem *qtOther = static_cast<const CargoTestTreeItem *>(other);
        return qtOther->inherited() == inherited && qtOther->name() == name;
    });
}

QString CargoTestTreeItem::nameSuffix() const
{
    static QString inheritedSuffix = QString(" [")
                + QCoreApplication::translate("cargotestTreeItem", "inherited")
                + QString("]");
