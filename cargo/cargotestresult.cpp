...
CargoTestResult::CargoTestResult(const QString &id, const QString &projectFile, TestType type,
                           const QString &className)
    : TestResult(id, className), m_projectFile(projectFile), m_type(type)
{
}

const QString CargoTestResult::outputString(bool selected) const
{
...
bool CargoTestResult::isDirectParentOf(const TestResult *other, bool *needsIntermediate) const
{
    if (!TestResult::isDirectParentOf(other, needsIntermediate))
        return false;
    const CargoTestResult *qtOther = static_cast<const CargoTestResult *>(other);
...
bool CargoTestResult::isIntermediateFor(const TestResult *other) const
{
...
TestResult *CargoTestResult::createIntermediateResultFor(const TestResult *other)
{
    QTC_ASSERT(other, return nullptr);
    const CargoTestResult *qtOther = static_cast<const CargoTestResult *>(other);
    CargoTestResult *intermediate = new CargoTestResult(qtOther->id(), qtOther->m_projectFile,
                                                  m_type, qtOther->name());
...
const TestTreeItem *CargoTestResult::findTestTreeItem() const
{
    Utils::Id id;
...
bool CargoTestResult::matches(const TestTreeItem *item) const
{
...
bool CargoTestResult::matchesTestCase(const TestTreeItem *item) const
{
...
bool CargoTestResult::matchesTestFunction(const TestTreeItem *item) const
{
    TestTreeItem *parentItem = item->parentItem();
    TestTreeItem::Type type = item->type();
